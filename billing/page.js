    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',fontFamily:'Inter,sans-serif'}}>
      <nav style={{background:'#fff',borderBottom:'1px solid #e2e8f0',height:56,display:'flex',alignItems:'center',padding:'0 24px',gap:16}}>
        <Link href="/dashboard" style={{display:'flex',alignItems:'center',gap:8,textDecoration:'none'}}>
          ...
          <span style={{fontWeight:700,color:'#0f172a',fontSize:14}}>HexorIQ</span>
        </Link>
        <div style={{flex:1}}/>
          ...
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:16}}>
              {PLANS.map(plan => {
                ...
                return (
                  <div key={plan.id} style={{borderRadius:14,border: plan.highlight ? '2px solid #dc2626' : isCurrent ? '2px solid #059669' : '1px solid #e2e8f0',padding:22,background: plan.highlight ? '#f5f3ff' : isCurrent ? '#f0fdf4' : '#fff',position:'relative'}}>
                    {plan.highlight && !isCurrent && <div style={{position:'absolute',top:-12,left:'50%',transform:'translateX(-50%)',background:'#dc2626',color:'#fff',fontSize:10,fontWeight:700,padding:'3px 12px',borderRadius:10,whiteSpace:'nowrap'}}>Most Popular</div>}
                    ...
                    <button onClick={() => handleUpgrade(plan.id)} disabled={isCurrent || plan.id === 'free' || loading}
                      style={{width:'100%',padding:'9px',borderRadius:8,border:'none',
                        background: isCurrent ? '#059669' : plan.highlight ? '#dc2626' : '#f8fafc',
                        color: isCurrent || plan.highlight ? '#fff' : '#dc2626',
            ...
            {invoices.length === 0 ? (
              ...
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid #f8fafc',fontSize:13}}>
                ...
                <div style={{display:'flex',gap:12,alignItems:'center'}}>
                  ...
                  {inv.pdf && <a href={inv.pdf} target="_blank" rel="noopener noreferrer" style={{color:'#dc2626',fontSize:11,fontWeight:600,textDecoration:'none'}}>↓ PDF</a>}
                </div>
              </div>
            ))}
          </div>
