...
const GOOGLE_AUTH_URL = `https://wdeilbhnsdlmckovicqy.supabase.co/auth/v1/authorize?provider=google&redirect_to=https%3A%2F%2Fapp.hexoriq.com%2Fauth%2Fcallback&scopes=email+profile`
...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = tab === 'login'
        ? { email: form.email, password: form.password, product: 'hexoriq' }
        : { email: form.email, password: form.password, full_name: form.name, source_product: 'hexoriq' }
      ...
      document.cookie = `hex_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `hex_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
      ...
      if (tab === 'signup') {
        fetch(`${process.env.NEXT_PUBLIC_DB_API_URL}/email/welcome`, {
          ...
          body: JSON.stringify({ product: 'hexoriq', to: form.email, name: fullName, email: form.email })
  ...
  const leftPanel = (
    <div style={{width:420,flexShrink:0,background:'#dc2626',display:'flex',flexDirection:'column',padding:'40px',position:'relative',overflow:'hidden'}}>
      ...
      <Link href="/" style={{display:'flex',alignItems:'center',gap:10,marginBottom:48,zIndex:1,textDecoration:'none'}}>
        <div style={{width:36,height:36,borderRadius:8,background:'rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:800,color:'#fff'}}>H</div>
        <span style={{fontSize:20,fontWeight:800,color:'#fff'}}>HexorIQ</span>
      </Link>
      <div style={{zIndex:1,marginBottom:40}}>
        ...
        <p style={{fontSize:14,color:'rgba(255,255,255,0.7)',lineHeight:1.6}}>Paste your reviews and get an instant AI summary — themes, sentiment, and action points — in seconds.</p>
      </div>
      <div style={{zIndex:1,marginTop:'auto',background:'rgba(255,255,255,0.1)',borderRadius:12,padding:20,border:'1px solid rgba(255,255,255,0.15)'}}>
        <p style={{fontSize:13,color:'rgba(255,255,255,0.9)',lineHeight:1.6,marginBottom:10,fontStyle:'italic'}}>"HexorIQ saves me hours every week."</p>
        ...
        <div style={{width:'100%',maxWidth:400}}>
          {isMobile && (
            <Link href="/" style={{display:'flex',alignItems:'center',gap:8,marginBottom:24,textDecoration:'none',justifyContent:'center'}}>
              <div style={{width:32,height:32,borderRadius:8,background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:800,color:'#fff'}}>H</div>
              <span style={{fontWeight:800,fontSize:18,color:'#0f172a'}}>HexorIQ</span>
            </Link>
          )}
          ...
          <p style={{fontSize:14,color:'#64748b',marginBottom:20}}>
            {tab === 'login' ? `Sign in to your HexorIQ account.` : 'Start your free trial today.'}
          </p>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:13,borderRadius:10,border:'none',background:'#dc2626',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:16,opacity:loading?0.7:1}}>
