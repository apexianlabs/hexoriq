...
const TESTIMONIALS = [
  { quote: 'HexorIQ saves me at least 3 hours a week. It handles the work I used to dread.', name: 'Sarah K.', role: 'Freelance Designer' },
    ...
    <div style={{minHeight:'100vh',fontFamily:'Inter,system-ui,sans-serif',background:'#fff',color:'#0f172a'}}>
      ...
      <nav style={{position:'sticky',top:0,zIndex:100,background:'rgba(255,255,255,0.92)',backdropFilter:'blur(12px)',borderBottom:'1px solid rgba(0,0,0,0.06)',padding:'0 24px',height:60,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{width:30,height:30,borderRadius:8,background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:800,color:'#fff',letterSpacing:'-0.5px'}}>H</div>
          <span style={{fontWeight:800,fontSize:17,color:'#0f172a',letterSpacing:'-0.3px'}}>HexorIQ</span>
        </div>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          ...
          <Link href="/signup" style={{padding:'8px 18px',fontSize:13,background:'#dc2626',color:'#fff',textDecoration:'none',borderRadius:8,fontWeight:600,letterSpacing:'-0.2px'}}>Start free</Link>
        </div>
      </nav>
      ...
      <section style={{padding:'96px 24px 80px',textAlign:'center',background:'linear-gradient(180deg,#fafbff 0%,#fff 60%)'}}>
        ...
        <p style={{fontSize:'clamp(16px,2vw,19px)',color:'#64748b',maxWidth:520,margin:'0 auto 36px',lineHeight:1.65,fontWeight:400}}>
          Paste your reviews and get an instant AI summary — themes, sentiment, and action points — in seconds.
        </p>
        <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap',marginBottom:20}}>
          <Link href="/generate" style={{padding:'14px 28px',background:'#dc2626',color:'#fff',textDecoration:'none',borderRadius:10,fontWeight:700,fontSize:15,letterSpacing:'-0.2px',boxShadow:'0 4px 14px rgba(0,0,0,0.15)'}}>
            Try it free →
          </Link>
        ...
        <div style={{maxWidth:860,margin:'56px auto 0',borderRadius:16,border:'1px solid #e8ecf0',boxShadow:'0 24px 64px rgba(0,0,0,0.08)',overflow:'hidden',background:'#fff'}}>
          <div style={{background:'#f8fafc',borderBottom:'1px solid #e8ecf0',padding:'12px 20px',display:'flex',alignItems:'center',gap:8}}>
            ...
            <div style={{flex:1,background:'#fff',borderRadius:6,height:26,margin:'0 12px',display:'flex',alignItems:'center',padding:'0 12px',border:'1px solid #e8ecf0'}}>
              <span style={{fontSize:11,color:'#94a3b8'}}>app.hexoriq.com/generate</span>
            </div>
          </div>
            ...
            <div style={{width:'100%',maxWidth:600}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
                <div style={{background:'#f8fafc',borderRadius:10,border:'1px solid #e8ecf0',padding:20}}>
                  <p style={{fontSize:10,fontWeight:700,color:'#dc2626',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:12}}>Input</p>
                  ...
                  <div style={{background:'#dc2626',color:'#fff',borderRadius:7,padding:'9px',textAlign:'center',fontWeight:700,fontSize:12,marginTop:12}}>Generate →</div>
                </div>
                <div style={{background:'#f8fafc',borderRadius:10,border:'1px solid #e8ecf0',padding:20}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                    <p style={{fontSize:10,fontWeight:700,color:'#dc2626',textTransform:'uppercase',letterSpacing:'0.06em'}}>Result</p>
                  ...
                  {['Professional output','Fully structured','Ready to use'].map((f,i) => (
                    <div key={i} style={{background:'#fff',border:'1px solid #e8ecf0',borderRadius:6,padding:'8px 10px',marginBottom:8,fontSize:12,color:'#374151',display:'flex',alignItems:'center',gap:6}}>
                      <span style={{color:'#dc2626',fontSize:10}}>✓</span>{f}
                    </div>
                  ))}
                  <div style={{display:'flex',gap:8,marginTop:12}}>
                    <div style={{flex:1,background:'#dc2626',color:'#fff',borderRadius:6,padding:'7px',textAlign:'center',fontSize:10,fontWeight:700}}>DOCX</div>
        ...
        <div style={{maxWidth:900,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'center',gap:40,flexWrap:'wrap'}}>
          {[['2+ hrs','Saved per task'],['30 sec','To first result'],['3 free','No card needed']].map(([stat,label]) => (
            <div key={stat} style={{textAlign:'center'}}>
              <p style={{fontSize:22,fontWeight:800,color:'#dc2626',letterSpacing:'-0.5px',marginBottom:2}}>{stat}</p>
      ...
      <section id="features" style={{padding:'80px 24px',maxWidth:1000,margin:'0 auto'}}>
        <div style={{textAlign:'center',marginBottom:52}}>
          <p style={{fontSize:11,fontWeight:700,color:'#dc2626',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>WHY HEXORIQ</p>
      ...
      <section style={{padding:'80px 24px',background:'#0f172a'}}>
        <div style={{maxWidth:1000,margin:'0 auto'}}>
          <div style={{textAlign:'center',marginBottom:48}}>
            <p style={{fontSize:11,fontWeight:700,color:'#dc2626',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>TRUSTED BY PROS</p>
            ...
            {TESTIMONIALS.map(t => (
              <div key={t.name} style={{background:'rgba(255,255,255,0.05)',borderRadius:14,border:'1px solid rgba(255,255,255,0.08)',padding:24}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:32,height:32,borderRadius:'50%',background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#fff'}}>{t.name[0]}</div>
                  <div>
      ...
      <section id="pricing" style={{padding:'80px 24px',maxWidth:900,margin:'0 auto'}}>
        <div style={{textAlign:'center',marginBottom:48}}>
          <p style={{fontSize:11,fontWeight:700,color:'#dc2626',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>PRICING</p>
          <h2 style={{fontSize:'clamp(28px,4vw,42px)',fontWeight:800,letterSpacing:'-1px',marginBottom:8,color:'#0f172a'}}>
            Start free. Scale when ready.
          </h2>
          <p style={{fontSize:15,color:'#64748b'}}>One saved hour pays for months of HexorIQ.</p>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:14}}>
          {[
            ...
            <div key={plan.name} style={{borderRadius:14,border: plan.highlight ? '2px solid #dc2626' : '1px solid #e8ecf0',padding:22,background: plan.highlight ? '#fafbff' : '#fff',position:'relative'}}>
              {plan.highlight && <div style={{position:'absolute',top:-12,left:'50%',transform:'translateX(-50%)',background:'#dc2626',color:'#fff',fontSize:10,fontWeight:700,padding:'3px 12px',borderRadius:10,whiteSpace:'nowrap',letterSpacing:'0.03em'}}>Most Popular</div>}
              ...
              <Link href="/signup" style={{display:'block',textAlign:'center',padding:'10px',borderRadius:8,
                background: plan.highlight ? '#dc2626' : '#f8fafc',
      ...
      {/* Final CTA */}
      <section style={{padding:'80px 24px',textAlign:'center',background:'linear-gradient(135deg,#dc2626 0%,#dc2626dd 100%)'}}>
        ...
        <p style={{fontSize:16,color:'rgba(255,255,255,0.8)',marginBottom:32,maxWidth:400,margin:'0 auto 32px'}}>
          Join professionals who use HexorIQ to work faster and earn more.
        </p>
        <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap',marginBottom:16}}>
          <Link href="/signup" style={{padding:'14px 28px',background:'#fff',color:'#dc2626',textDecoration:'none',borderRadius:10,fontWeight:700,fontSize:15,letterSpacing:'-0.2px'}}>
            Start free →
          </Link>
      ...
      <footer style={{padding:'28px 24px',borderTop:'1px solid #f1f5f9',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12,background:'#fff'}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{width:22,height:22,borderRadius:5,background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:800,color:'#fff'}}>H</div>
          <span style={{fontWeight:700,fontSize:13,color:'#0f172a'}}>HexorIQ</span>
        </div>
        <p style={{fontSize:12,color:'#94a3b8'}}>© {new Date().getFullYear()} HexorIQ. Built on Claude AI.</p>
