...
useEffect(() => {
  ...
  const handleLogout = () => {
    document.cookie = 'hex_token=; path=/; max-age=0'
    document.cookie = 'hex_user=; path=/; max-age=0'
    router.push('/login')
  }
    ...
    <div style={{width:220,background:'#0f172a',display:'flex',flexDirection:'column',minHeight:'100vh',flexShrink:0,position:'relative'}}>
      ...
      <div style={{padding:'20px 16px 16px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
        <Link href="/" style={{display:'flex',alignItems:'center',gap:9,textDecoration:'none'}}>
          <div style={{width:28,height:28,borderRadius:7,background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,color:'#fff',flexShrink:0}}>H</div>
          <div>
            <p style={{fontSize:13,fontWeight:800,color:'#fff',lineHeight:1,letterSpacing:'-0.3px'}}>HexorIQ</p>
            <p style={{fontSize:10,color:'#475569',lineHeight:1,marginTop:2}}>Paste your reviews and</p>
          </div>
        </Link>
      </div>
      ...
      <div style={{padding:'12px 12px 8px'}}>
        <Link href="/generate" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:7,background:'#dc2626',color:'#fff',textDecoration:'none',borderRadius:8,padding:'10px',fontSize:13,fontWeight:600,letterSpacing:'-0.2px'}}>
          <span style={{fontSize:16,lineHeight:1}}>+</span> New Summary
        </Link>
      </div>
      ...
      <div style={{padding:'4px 8px',flex:1}}>
        {[
          ...
          { icon:'□', label:'Summaries', href:'/items' },
      ...
      <div style={{padding:'12px',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
        <div style={{background:'rgba(255,255,255,0.04)',borderRadius:10,padding:'10px 12px',marginBottom:8,border:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
            ...
            <Link href="/billing" style={{fontSize:10,color:'#dc2626',textDecoration:'none',fontWeight:600}}>Upgrade</Link>
          </div>
          <div style={{background:'rgba(255,255,255,0.08)',borderRadius:3,height:3,marginBottom:6}}>
            <div style={{background:'#dc2626',borderRadius:3,height:3,width:`${Math.min(100,(items.length/3)*100)}%`,transition:'width 0.3s'}}/>
          </div>
        ...
        <button onClick={handleLogout}
          ...
          <div style={{width:28,height:28,borderRadius:7,background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0}}>
            {getInitials(user)}
          </div>
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',fontFamily:'Inter,system-ui,sans-serif',background:'#f8fafc'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
        ...
        <div style={{background:'#fff',borderBottom:'1px solid #f1f5f9',height:56,display:'flex',alignItems:'center',padding:'0 20px',gap:12,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:11,color:'#94a3b8',marginTop:1}}>Here's your HexorIQ overview.</p>
          </div>
          <Link href="/generate"
            style={{background:'#dc2626',color:'#fff',padding:'7px 16px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none',letterSpacing:'-0.2px',flexShrink:0}}>
            + New
          </Link>
        </div>
        ...
        <div style={{flex:1,padding:'20px',overflowY:'auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)',gap:12,marginBottom:20}}>
            {[
              { label:'Total', value: items.length, sub: 'All time', color:'#dc2626', icon:'📄' },
          ...
          <div style={{display:'grid',gridTemplateColumns: isMobile ? '1fr' : '1fr 300px',gap:16}}>
            ...
            <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <div style={{padding:'16px 20px',borderBottom:'1px solid #f8fafc',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <p style={{fontSize:14,fontWeight:700,color:'#0f172a',letterSpacing:'-0.2px'}}>Recent Summaries</p>
                <Link href="/generate" style={{fontSize:12,color:'#dc2626',textDecoration:'none',fontWeight:600,background:'rgba(0,0,0,0.03)',padding:'4px 10px',borderRadius:6}}>+ New</Link>
              </div>
        ...
        {usageData && usageData.plan === 'free' && (
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:16}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <a href="/billing" style={{fontSize:11,fontWeight:700,color:'#dc2626',textDecoration:'none'}}>Upgrade →</a>
            </div>
          </div>
        )}
        {loading ? (
                ...
                <div style={{padding:'48px 24px',textAlign:'center'}}>
                  ...
                  <p style={{fontSize:14,fontWeight:600,color:'#0f172a',marginBottom:6}}>No Summaries yet</p>
                  <p style={{fontSize:13,color:'#94a3b8',marginBottom:20}}>Generate your first one in seconds</p>
                  <Link href="/generate" style={{background:'#dc2626',color:'#fff',padding:'9px 20px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none'}}>
                    Get started →
                  </Link>
                </div>
                ...
                <div key={item.id}
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0,flex:1}}>
                    <div style={{width:34,height:34,borderRadius:9,background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:'#dc2626',flexShrink:0,border:'1px solid rgba(0,0,0,0.04)'}}>
                  ...
                  <Link href={`/items/${item.id}`}
                    style={{color:'#dc2626',fontSize:11,fontWeight:600,textDecoration:'none',padding:'5px 10px',border:'1px solid rgba(0,0,0,0.08)',borderRadius:6,flexShrink:0,marginLeft:12,background:'#fff'}}>
                    View →
                  </Link>
                </div>
              ))}
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {/* Quick action */}
              <div style={{background:'linear-gradient(135deg,#dc2626 0%,#dc2626cc 100%)',borderRadius:14,padding:20,color:'#fff'}}>
                <p style={{fontSize:13,fontWeight:700,marginBottom:4,letterSpacing:'-0.2px'}}>Ready to generate?</p>
                <p style={{fontSize:12,color:'rgba(255,255,255,0.75)',marginBottom:14,lineHeight:1.5}}>Paste your reviews and get an instant AI summary — themes, sentiment, and action points — in seconds.</p>
              ...
              <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',padding:20,flex:1,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                ...
                {items.length === 0 ? (
                  ...
                  <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,marginBottom:12}}>
                    <div style={{width:26,height:26,borderRadius:'50%',background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,flexShrink:0,color:'#dc2626',fontWeight:700,border:'1px solid rgba(0,0,0,0.04)'}}>+</div>
                    <div style={{minWidth:0}}>
              ...
              {items.length >= 2 && (
                <div style={{background:'#fafafa',borderRadius:14,border:'1px solid #f1f5f9',padding:20}}>
                  ...
                  <p style={{fontSize:12,color:'#64748b',marginBottom:12,lineHeight:1.5}}>Get unlimited Summaries from $19/month.</p>
                  <Link href="/billing" style={{display:'block',textAlign:'center',background:'#dc2626',color:'#fff',textDecoration:'none',borderRadius:8,padding:'8px',fontSize:12,fontWeight:700}}>
                    View plans →
                  </Link>
                </div>
              )}
      ...
      <div style={{borderTop:'1px solid #e2e8f0',padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#fff',marginTop:'auto'}}>
        <span style={{fontSize:11,color:'#94a3b8'}}>© 2026 HexorIQ</span>
        <div style={{display:'flex',gap:20}}>
          <a href="https://hexoriq.com/privacy" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Privacy</a>
          <a href="https://hexoriq.com/terms" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Terms</a>
          <a href="https://hexoriq.com/support" style={{fontSize:11,color:'#94a3b8',textDecoration:'none'}} target="_blank" rel="noreferrer">Support</a>
        </div>
      </div>
    </div>
  )
